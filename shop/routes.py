...
from . import app, csrf, db, stripe
...
def create_checkout_session():
    ...
    try:
        ...
        total = sum(i['price'] * i['quantity'] for i in session['cart'].values())
        if total > 999999.99:
            flash('Total amount must be less than $999,999.99.', 'danger')
        else:
            flash('An invalid request occurred.', 'danger')
        return redirect(url_for('cart'))
    except Exception as e:
        flash(f'{str(e)}', 'danger')
        return redirect(url_for('cart'))
...
def fulfill_order(stripe_session, status='Paid'):
    ...
    address_dict = stripe_session['shipping_details']['address']
    postal_code = address_dict['postal_code']
    address = ''.join(
        address_dict[key]
        for key in ('state', 'city', 'line1', 'line2')
